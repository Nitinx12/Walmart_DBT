...
phone_split AS (
    ...
    SELECT
        *,
        ...
        NULLIF(SPLIT_PART(LOWER(TRIM(phone::VARCHAR)), 'x', 2), '') AS phone_ext_part
    FROM deduplicated
    WHERE rnk = 1
),
phone_digits AS (
    ...
    SELECT
        *,
        ...
        REGEXP_REPLACE(COALESCE(phone_ext_part, ''), '[^0-9]', '', 'g') AS phone_ext_digits
    FROM phone_split
),
cleaned AS (
    SELECT
        ...
        CASE
            ...
            THEN '+1-' || SUBSTRING(RIGHT(phone_digits, 10), 1, 3) || '-' ||
                           SUBSTRING(RIGHT(phone_digits, 10), 4, 3) || '-' ||
                           SUBSTRING(RIGHT(phone_digits, 10), 7, 4)
            ELSE NULL  -- fewer than 10 digits = not a valid number, flagged as NULL rather than stored as garbage
        END                                                  AS phone,
